...
def first_anc_pass(plink2,directory,header,ancestry_file_path,ancestry_matrix,ancestries,ancestry_file_anccolname='race_cat',ancestry_file_idcolname='Subject ID',input_bfile=None):
    pheno=pd.read_table(ancestry_file_path, sep=',')
    data_dir = directory+header
    FAM_phenos=pd.read_table(data_dir + ".fam",header=None,delim_whitespace=True)
    #FAM_phenos['final_ID']=FAM_phenos[1].str.split('-',expand=True)[0].astype(int)
    FAM_phenos['final_ID']=FAM_phenos[1]
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Run QC pipeline with PLINK for genomic data.')
    parser.add_argument('--plink', required=False, help='Path to PLINK executable')
    parser.add_argument('--directory', required=False, help='Directory with starting PLINK bed/bim/fam files')
    parser.add_argument('--header', required=True, help='Header of starting PLINK files')
    parser.add_argument('--ancestry_file_path', required=True, help='Path to ancestry file with subject IDs and categories')
    parser.add_argument('--ancestry_matrix', required=False, type=str, help='JSON string specifying ancestry categories')
    parser.add_argument('--pheno_files', required=True, nargs='+', help='List of phenotype files for test-missing filtering')
    parser.add_argument('--pca_bimfile', help='Optional PCA BIM file path')
    parser.add_argument('--miss_cutoff', type=float, default=0.05, help='Missing rate cutoff for SNPs')
    parser.add_argument('--test_missing_cutoff', type=float, default=0.00005, help='Cutoff for test-missing filtering')
    parser.add_argument('--maf_cutoff', type=float, default=0.0005, help='Minor allele frequency cutoff')
    parser.add_argument('--hwe_cutoff', type=float, default=1e-10, help='Hardy-Weinberg equilibrium cutoff')
    parser.add_argument('--mds', action='store_true', help='Run MDS as part of QC')
    args = parser.parse_args()

    plink2 = 'plink' # plink executable
    directory = './' # directory containing starting plink bed/bim/fam files
    header = args.header # header of starting plink bed/bim/fam files
    ancestry_file_path = args.ancestry_file_path # tab-delimited file containing
    ...
    ancestry_matrix = {'EU':['W']} # for testing subset to EU
    ...
    pheno_files = args.pheno_files # for testing subset to test data
